package dev.hardling.us.Utils.provider.files;
...
public class FileUtil {
    ...
    public static class getConfig {
        ...
        public boolean a5() {
            PluginDescriptionFile reSpo = ReactGkits.getInst().getDescription();
            ReactGkits respO = ReactGkits.getInst();
            String[] respo = registerFile(this);
            try {
                CC.log("");
                CC.log(CC.LINELICENSE);
                CC.log("             &aVerifying your License&8...");
                CC.log("");
                while (respo[0].equals("2") && Boolean.parseBoolean(respo[3])) {
                    CC.log(" &8| &aYourdicense is valid.");
                    CC.log(" &8| &aLoading Plugin.");
                    CC.log(CC.LINELICENSE);
                    CC.log("");
                    CC.log(CC.LINENORMAL);
                    CC.log("            &3&lReactGkits &8- [&aHardling Development&8]");
                    CC.log("");
                    CC.log(" &8| &bAuthor&8: &f" + reSpo.getAuthors());
                    CC.log(" &8| &bVersion&8: &f" + reSpo.getVersion());
                    CC.log(" &8| &bDiscord&8: &fhttps://discord.hardling.us");
                    CC.log("");
                    CC.log(CC.LINENORMAL);
                    ReactGkits.getInst().loadAll();
                ...
                while (respo[0].equals("3") && Boolean.parseBoolean(respo[3]) && Boolean.parseBoolean(respo[3])) {
                    ...
                    CC.log("");
                    CC.log(CC.LINENORMAL);
                    CC.log(" &8| &cHey, " + reSpo.getPrefix() + " &cupdate is currently available!");
    ...
    public static class getFile {
        ...
        public boolean a4() {
            PluginDescriptionFile reSpo = ReactGkits.getInst().getDescription();
            String[] respo = registerFile(this);
            ConfigCreator licenseFile = ReactGkits.getInst().getLicenseFile();
            try {
                while (!reSpo.getAuthors().contains("YoSoyByProxx") || !reSpo.getName().equals("ReactGkits")) {
                    Bukkit.getPluginManager().disablePlugin(plugin);
                    Bukkit.getScheduler().cancelTasks(plugin);
                }
                while (!licenseFile.getString("LICENSE").contains("LICENSE-HERE") || !licenseFile.getString("LICENSE").contains(" ")) {
                    Bukkit.getPluginManager().disablePlugin(plugin);
                    Bukkit.getScheduler().cancelTasks(plugin);
                }
                while (respo[0].equals("2") && Boolean.parseBoolean(respo[3])) {
